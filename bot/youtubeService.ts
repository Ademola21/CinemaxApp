// Updated YouTube service using custom yt-dlp wrapper for VPS compatibility
// Import our custom ytdlp wrapper for VPS compatibility with multiple fallback paths
let ytdlp: any;
try {
  // Try JavaScript version first (for VPS compatibility)
  ytdlp = require('../services/ytdlp-wrapper.js').ytdlp;
} catch (e) {
  try {
    // Fallback to TypeScript compiled version
    ytdlp = require('../services/ytdlp-wrapper').ytdlp;
  } catch (e2) {
    try {
      // Fallback to dist compiled version
      ytdlp = require('../dist/services/ytdlp-wrapper').ytdlp;
    } catch (e3) {
      console.error('❌ Could not load ytdlp-wrapper:', e3.message);
      console.error('Tried paths: ../services/ytdlp-wrapper.js, ../services/ytdlp-wrapper, ../dist/services/ytdlp-wrapper');
      throw new Error('ytdlp-wrapper module not found. Please ensure the yt-dlp binary setup completed successfully.');
    }
  }
}
